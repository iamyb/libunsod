...
iperf_sctp_accept(struct iperf_test * test)
{
    ...
    len = sizeof(addr);
    s = ud_accept(test->listener, (struct sockaddr *) &addr, &len);
    if (s < 0) {
...
iperf_sctp_listen(struct iperf_test *test)
{
    ...
    if ((s = ud_socket(res->ai_family, SOCK_STREAM, IPPROTO_SCTP)) < 0) {
        freeaddrinfo(res);
        i_errno = IESTREAMLISTEN;
        return -1;
    }
...
iperf_sctp_connect(struct iperf_test *test)
{
    ...
    s = ud_socket(server_res->ai_family, SOCK_STREAM, IPPROTO_SCTP);
    if (s < 0) {
        if (test->bind_address)
            freeaddrinfo(local_res);
    ...
    if (test->no_delay != 0) {
        opt = 1;
        if (ud_setsockopt(s, IPPROTO_SCTP, SCTP_NODELAY, &opt, sizeof(opt)) < 0) {
            ud_close(s);
            freeaddrinfo(server_res);
    ...
    if ((test->settings->mss >= 512 && test->settings->mss <= 131072)) {
        ...
        if (ud_setsockopt(s, IPPROTO_SCTP, SCTP_MAXSEG, &av, sizeof(av)) < 0) {
            ud_close(s);
            freeaddrinfo(server_res);
            i_errno = IESETMSS;
            return -1;
         ...
         */
        if (ud_setsockopt(s, IPPROTO_SCTP, SCTP_MAXSEG, &opt, sizeof(opt)) < 0 &&
                errno != ENOPROTOOPT) {
            ud_close(s);
            freeaddrinfo(server_res);
            i_errno = IESETMSS;
    ...
    if (test->settings->num_ostreams > 0) {
        ...
        if (ud_setsockopt(s, IPPROTO_SCTP, SCTP_INITMSG, &initmsg, sizeof(struct sctp_initmsg)) < 0) {
            ud_close(s);
            freeaddrinfo(server_res);
     ...
     */
    opt = 0;
    if (ud_setsockopt(s, IPPROTO_SCTP, SCTP_DISABLE_FRAGMENTS, &opt, sizeof(opt)) < 0 &&
            errno != ENOPROTOOPT) {
        ud_close(s);
        freeaddrinfo(server_res);
...
iperf_sctp_bindx(struct iperf_test *test, int s, int is_server)
{
    ...
    if (!is_server) {
        ...
        if (ud_bind(s, (struct sockaddr *)ai->ai_addr, ai->ai_addrlen) < 0) {
            i_errno = IESETSCTPBINDX;
            retval = -1;
            goto out;
        }
