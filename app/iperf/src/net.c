...
netdial(int domain, int proto, char *local, int local_port, char *server, int port)
{
    ...
    s = ud_socket(server_res->ai_family, proto, 0);
    if (s < 0) {
        if (local)
...
netannounce(int domain, int proto, char *local, int port)
{
    ...
    s = ud_socket(res->ai_family, proto, 0);
    if (s < 0) {
        freeaddrinfo(res);
        return -1;
    }
    ...
    opt = 1;
    if (ud_setsockopt(s, SOL_SOCKET, SO_REUSEADDR,
...
getsock_tcp_mss(int inSock)
{
    ...
    /* query for mss */
    len = sizeof(mss);
    rc = ud_getsockopt(inSock, IPPROTO_TCP, TCP_MAXSEG, (char *)&mss, &len);
    if (rc == -1) {
...
set_tcp_options(int sock, int no_delay, int mss)
{
    ...
    if (no_delay == 1) {
        len = sizeof(no_delay);
        rc = ud_setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char *)&no_delay, len);
        if (rc == -1) {
    ...
    if (mss > 0) {
        ...
        new_mss = mss;
        len = sizeof(new_mss);
        rc = ud_setsockopt(sock, IPPROTO_TCP, TCP_MAXSEG, (char *)&new_mss, len);
        if (rc == -1) {
            perror("setsockopt TCP_MAXSEG");
            return -1;
        }
        /* verify results */
        rc = ud_getsockopt(sock, IPPROTO_TCP, TCP_MAXSEG, (char *)&new_mss, &len);
        if (rc == -1) {
            perror("getsockopt TCP_MAXSEG");
            return -1;
        }
...
setnonblocking(int fd, int nonblocking)
{
    ...
    if (nonblocking)
        ...
        newflags = flags & ~((int) O_NONBLOCK);
    if (newflags != flags)
        if (ud_fcntl(fd, F_SETFL, newflags) < 0) {
...
getsockdomain(int sock)
{
    ...
    if (ud_getsockname(sock, (struct sockaddr *)&sa, &len) < 0) {
        return -1;
    }
