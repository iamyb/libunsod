...
iperf_tcp_accept(struct iperf_test * test)
{
    ...
    len = sizeof(addr);
    if ((s = ud_accept(test->listener, (struct sockaddr *) &addr, &len)) < 0) {
...
iperf_tcp_listen(struct iperf_test *test)
{
    ...
    if (test->no_delay || test->settings->mss || test->settings->socket_bufsize) {
        ...
        if ((s = ud_socket(res->ai_family, SOCK_STREAM, 0)) < 0) {
            freeaddrinfo(res);
            i_errno = IESTREAMLISTEN;
            return -1;
        }
        ...
        if (test->no_delay) {
            opt = 1;
            if (ud_setsockopt(s, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) < 0) {
                saved_errno = errno;
                ud_close(s);
                freeaddrinfo(res);
                errno = saved_errno;
        ...
        if ((opt = test->settings->mss)) {
            if (ud_setsockopt(s, IPPROTO_TCP, TCP_MAXSEG, &opt, sizeof(opt)) < 0) {
                saved_errno = errno;
                ud_close(s);
                freeaddrinfo(res);
                errno = saved_errno;
        ...
        if ((opt = test->settings->socket_bufsize)) {
            if (ud_setsockopt(s, SOL_SOCKET, SO_RCVBUF, &opt, sizeof(opt)) < 0) {
                saved_errno = errno;
                ud_close(s);
                freeaddrinfo(res);
                errno = saved_errno;
            ...
            if (ud_setsockopt(s, SOL_SOCKET, SO_SNDBUF, &opt, sizeof(opt)) < 0) {
                saved_errno = errno;
                ud_close(s);
                freeaddrinfo(res);
                errno = saved_errno;
        ...
        if (test->debug) {
            ...
            if (ud_getsockopt(s, SOL_SOCKET, SO_SNDBUF, &opt, &optlen) < 0) {
                saved_errno = errno;
                ud_close(s);
                freeaddrinfo(res);
                errno = saved_errno;
        ...
        if (test->settings->fqrate) {
            ...
            if (fqrate > 0) {
                ...
                if (ud_setsockopt(s, SOL_SOCKET, SO_MAX_PACING_RATE, &fqrate, sizeof(fqrate)) < 0) {
        ...
        opt = 1;
        if (ud_setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
            saved_errno = errno;
            ud_close(s);
            freeaddrinfo(res);
            errno = saved_errno;
        ...
        if (res->ai_family == AF_INET6 && (test->settings->domain == AF_UNSPEC || test->settings->domain == AF_INET)) {
            if (test->settings->domain == AF_UNSPEC)
                ...
                opt = 1;
            if (ud_setsockopt(s, IPPROTO_IPV6, IPV6_V6ONLY,
        ...
        if (ud_bind(s, (struct sockaddr *) res->ai_addr, res->ai_addrlen) < 0) {
            saved_errno = errno;
            ud_close(s);
            freeaddrinfo(res);
            errno = saved_errno;
...
iperf_tcp_connect(struct iperf_test *test)
{
    ...
    if ((s = ud_socket(server_res->ai_family, SOCK_STREAM, 0)) < 0) {
        if (test->bind_address)
            freeaddrinfo(local_res);
    ...
    if (test->bind_address) {
        ...
        if (ud_bind(s, (struct sockaddr *) local_res->ai_addr, local_res->ai_addrlen) < 0) {
            saved_errno = errno;
            ud_close(s);
            freeaddrinfo(local_res);
            freeaddrinfo(server_res);
    ...
    optlen = sizeof(opt);
    if (ud_getsockopt(s, SOL_SOCKET, SO_RCVBUF, &opt, &optlen) < 0) {
        saved_errno = errno;
        ud_close(s);
        freeaddrinfo(server_res);
        errno = saved_errno;
    ...
    if (test->settings->flowlabel) {
        if (server_res->ai_addr->sa_family != AF_INET6) {
            ...
            if (ud_setsockopt(s, IPPROTO_IPV6, IPV6_FLOWLABEL_MGR, freq, freq_len) < 0) {
                saved_errno = errno;
                ud_close(s);
                freeaddrinfo(server_res);
                errno = saved_errno;
            ...
            opt = 1;
            if (ud_setsockopt(s, IPPROTO_IPV6, IPV6_FLOWINFO_SEND, &opt, sizeof(opt)) < 0) {
                saved_errno = errno;
                ud_close(s);
                freeaddrinfo(server_res);
                errno = saved_errno;
    ...
    if (ud_connect(s, (struct sockaddr *) server_res->ai_addr, server_res->ai_addrlen) < 0 && errno != EINPROGRESS) {
        saved_errno = errno;
        ud_close(s);
        freeaddrinfo(server_res);
        errno = saved_errno;
