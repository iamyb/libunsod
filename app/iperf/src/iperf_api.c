...
iperf_on_connect(struct iperf_test *test)
{
    ...
    if (test->role == 'c') {
        ...
        len = sizeof(sa);
        ud_getpeername(test->ctrl_sck, (struct sockaddr *) &sa, &len);
...
iperf_init_stream(struct iperf_stream *sp, struct iperf_test *test)
{
    ...
    if (ud_getsockname(sp->socket, (struct sockaddr *) &sp->local_addr, &len) < 0) {
        i_errno = IEINITSTREAM;
        return -1;
    }
    len = sizeof(struct sockaddr_storage);
    if (ud_getpeername(sp->socket, (struct sockaddr *) &sp->remote_addr, &len) < 0) {
        i_errno = IEINITSTREAM;
        return -1;
    }
    ...
    if ((opt = test->settings->tos)) {
        if (getsockdomain(sp->socket) == AF_INET6) {
#ifdef IPV6_TCLASS
            if (ud_setsockopt(sp->socket, IPPROTO_IPV6, IPV6_TCLASS, &opt, sizeof(opt)) < 0) {
                i_errno = IESETCOS;
                return -1;
            }
#else
            ...
            if (ud_setsockopt(sp->socket, IPPROTO_IP, IP_TOS, &opt, sizeof(opt)) < 0) {
                i_errno = IESETTOS;
                return -1;
            }
        }
