...
iperf_create_streams(struct iperf_test *test)
{
    ...
    for (i = 0; i < test->num_streams; ++i) {
        ...
        if (test->protocol->id == Ptcp) {
            if (test->congestion) {
                if (ud_setsockopt(s, IPPROTO_TCP, TCP_CONGESTION, test->congestion, strlen(test->congestion)) < 0) {
                    ud_close(s);
            ...
            }
            {
                ...
                if (ud_getsockopt(s, IPPROTO_TCP, TCP_CONGESTION, ca, &len) < 0) {
                    ud_close(s);
                    i_errno = IESETCONGESTION;
                    return -1;
                }
...
iperf_handle_message_client(struct iperf_test *test)
{
    ...
    if ((rval = ud_read(test->ctrl_sck, (char*) &test->state, sizeof(signed char))) <= 0) {
        if (rval == 0) {
            i_errno = IECTRLCLOSE;
            return -1;
        } else {
...
iperf_connect(struct iperf_test *test)
{
    ...
    len = sizeof(opt);
    if (ud_getsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_MAXSEG, &opt, &len) < 0) {
        test->ctrl_sck_mss = 0;
    } else {
...
iperf_run_client(struct iperf_test * test)
{
    ...
    while (test->state != IPERF_DONE) {
        ...
        result = ud_select(test->max_fd + 1, &read_set, &write_set, NULL, timeout);
