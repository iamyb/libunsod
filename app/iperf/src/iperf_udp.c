...
iperf_udp_accept(struct iperf_test *test)
{
     ...
     */
    len = sizeof(sa_peer);
    if ((sz = ud_recvfrom(test->prot_listener, &buf, sizeof(buf), 0, (struct sockaddr *) &sa_peer, &len)) < 0) {
        i_errno = IESTREAMACCEPT;
        return -1;
    }

    if (ud_connect(s, (struct sockaddr *) &sa_peer, len) < 0) {
        i_errno = IESTREAMACCEPT;
        return -1;
    }
    ...
    if (ud_write(s, &buf, sizeof(buf)) < 0) {
...
iperf_udp_connect(struct iperf_test *test)
{
    ...
    tv.tv_sec = 30;
    tv.tv_usec = 0;
    ud_setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (struct timeval *)&tv, sizeof(struct timeval));
#endif
     ...
     */
    if ((sz = ud_recv(s, &buf, sizeof(buf), 0)) < 0) {
