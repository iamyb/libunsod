...
void* dh_alloc_desc(dh_rte_mbuf_desc* desc)
{
	...
	if(mb != NULL)
	{
		...
		if(((uint64_t)desc->debug_next - (uint64_t)desc->rm_base) != (0x50))
			printf("ddddddddddddddddddd dh_desc.rm_base %lx serialno %lx\n",  desc->rm_base, desc->debug_next);										
		//printf("dh_alloc_desc %lx\n", desc->rm_base);
	}
	else
	{
